...
BlockT::BlockT(const CellsArray& array, BlockRotationState startRotationState) 
    : Block(array, startRotationState)
{
}

bool BlockT::tryRotate(Grid& grid) {
...
bool BlockT::tryRotateReversed(Grid& grid) {
    auto& cells = getCells_mut();
    CellsArray expectedCells;
    Color color = cells[0]->getColor();
